...
public class AudioController : MonoBehaviour
{
    ...
    public void SoundPlay(string audioname, Action action = null, float volume = 1)
    {
        if (AudioDictionary.ContainsKey(audioname))
        {
            if (AudioDictionary[audioname] <= MaxAudioCount)
            {
                ...
                if (sound != null)
                {
                    ...
                    StartCoroutine(this.PlayClipEnd(sound, audioname, action));
    ...
    private IEnumerator PlayClipEndDestroy(AudioClip audioclip, GameObject soundobj)
    {
        ...
        {
            audiocliplength = audioclip.length * Time.timeScale;
    ...
    /// </summary>  
    /// <returns></returns>  
    private IEnumerator PlayClipEnd(AudioClip audioclip, string audioname, Action action)
    {
        if (audioclip != null)
        {
            yield return new WaitForSeconds(audioclip.length * Time.timeScale);
